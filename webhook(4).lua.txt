    ...
    if ok and r and r.data then
        for _,s in ipairs(r.data) do
            if s.playing<s.maxPlayers and s.id~=game.JobId then
                ...
                task.wait(10) rejoining=false return
            end
        end
    end
    TS:Teleport(PID,LP)
...
task.spawn(function()
    ...
    while true do
        ...
        if found then
            stuck=stuck+1
            if stuck>=15 then stuck=0 rejoinReason="Stuck" doRejoin() end
        else stuck=0 end
    end
end)
pcall(function()
    ...
    p.ChildAdded:Connect(function(c)
        if c.Name=="ErrorPrompt" then rejoinReason="Disconnect" doRejoin() end
    end)
end)
...
local function getStats()
    ...
    pcall(function()
        ...
        if SU and WU then
            ...
            pcall(function()
                s.luck="+"..string.format("%.0f",(SU:GetLuckMultiplier(LP,d,world,true)-1)*100).."%"
            end)
            pcall(function()
...
local function findIdFromChannel(name)
    local ok,r=pcall(function()
        return HS:RequestAsync({Url=WEBHOOK.."?limit=50",Method="GET",
    ...
    for _,msg in ipairs(data) do
        if msg.content then
            local id=msg.content:match("<!%-%- "..name..":(%d+) %-%->")
...
local function update()
    ...
    if not cachedId then
        cachedId=findIdFromChannel(LP.Name)
        print("[WH] Gefundene ID: "..(cachedId or "keine"))
    end
    local content=build(s,cachedId)
    if cachedId then
        ...
        if ok then print("[WH] Bearbeitet: "..LP.Name) return end
        cachedId=nil
    end
    local ok2,r2=pcall(function()
        return HS:RequestAsync({Url=WEBHOOK.."?wait=true",Method="POST",
            Headers={["Content-Type"]="application/json"},
            Body=HS:JSONEncode({content=build(s,"new")})})
    end)
    if ok2 and r2 and r2.Body then
        ...
        if data and data.id then
            cachedId=data.id
            local finalContent=build(s,cachedId)
            pcall(function()
                HS:RequestAsync({Url=WEBHOOK.."/messages/"..cachedId,Method="PATCH",
                    Headers={["Content-Type"]="application/json"},
                    Body=HS:JSONEncode({content=finalContent})})
            end)
            print("[WH] Neu erstellt: "..LP.Name.." ID:"..cachedId)
        end
    end
end
